...
def execute(args) -> int:
    logging.info(
        f"downloading and installing inspector-sbomgen version {args.sbomgen_version}"
    )
    ...
    pkg_vuln_markdown = write_pkg_vuln_report_markdown(
        args.out_scan_markdown, scan_result
    )
    ...
    set_github_actions_output(
        "inspector_dockerile_scan_results_csv", args.out_dockerfile_scan_csv
    )

    dockerfile.write_dockerfile_report_md(args.out_scan, args.out_dockerfile_scan_md)
    set_github_actions_output(
        "inspector_dockerile_scan_results_markdown", args.out_dockerfile_scan_md
    )
...
def post_dockerfile_step_summary(args, total_vulns):
    if args.display_vuln_findings == "enabled" and total_vulns > 0:
        logging.info(
            "posting Inspector Dockerfile scan findings to GitHub Actions step summary page"
        )
...
def download_install_sbomgen(sbomgen_version: str, install_dst: str) -> bool:
    ...
    if not sbomgen_arch:
        logging.error(
            f"expected a CPU architecture of x86_64, amd64, arm64, or aarch64, but received: {cpu_arch}"
        )
        sys.exit(1)
...
def get_vuln_counts(inspector_scan_path: str):
    ...
    if props is None:
        logging.error(
            f"expected metadata properties, but none were found in file {inspector_scan_path}"
        )
        return False, criticals, highs, mediums, lows, others
    ...
    for prop in props:
        ...
        if name is None:
            logging.error(
                f"expected property with 'name' key but none was found in file {inspector_scan_path}"
            )
            continue
        ...
        if value is None:
            logging.error(
                f"expected property with 'value' key but none was found in file {inspector_scan_path}"
            )
            continue
...
def install_sbomgen(args):
    ...
    if "Linux" in os_name:
        ret = download_install_sbomgen(
            args.sbomgen_version, "/usr/local/bin/inspector-sbomgen"
        )
        if not ret:
            return 1
...
def write_pkg_vuln_report_markdown(
    out_scan_markdown, scan_result: exporter.InspectorScanResult
):
    markdown = exporter.to_markdown(scan_result)

    logging.info(
        f"writing package vulnerability markdown report to: {out_scan_markdown}"
    )
...
def set_flag_if_vuln_threshold_exceeded(
    args, scan_result: exporter.InspectorScanResult
):
    is_exceeded = exceeds_threshold(
        scan_result.criticals,
        args.critical,
        scan_result.highs,
        args.high,
        scan_result.mediums,
...
def get_summarized_findings(scan_result: exporter.InspectorScanResult):
    ...
    artifact_type = (
        "repository"
        if scan_result.artifact_type.lower() == "directory"
        else scan_result.artifact_type
    )
    ...
    if len(scan_result.vulnerabilities) > 0:
        ...
        package_names = set()  # Use a set to ensure uniqueness
        for (
            vuln
        ) in scan_result.vulnerabilities:  # Assuming scan_result.vulnerabilities exists
            package_name = extract_package_name(vuln.installed_ver)
            if package_name:  # Only add non-empty package names
                package_names.add(package_name)

        # Sort package names alphabetically
        sorted_packages = sorted(package_names)

        # Append package names to the summary
        results += """
...
def extract_package_name(installed_ver: str) -> str:
    """
    Extracts the package name from the installed_ver field.
    """
    import re

    match = re.search(r"pkg:deb/debian/(?P<package_name>[^@]+)@", installed_ver)
    return match.group("package_name") if match else "Unknown Package"


def post_pkg_vuln_github_actions_step_summary(args, markdown):
    if args.display_vuln_findings == "enabled":
        logging.info(
            "posting Inspector scan findings to GitHub Actions step summary page"
        )
