...
from entrypoint import pkg_vuln, orchestrator
...
def to_markdown(scan_result: InspectorScanResult):
    ...
    markdown += create_packages_table(scan_result)
...
def create_header_info(scan_result: InspectorScanResult):
    ...
    artifact_type = (
        "repository"
        if scan_result.artifact_type == "directory"
...
def create_packages_table(scan_result: InspectorScanResult):
    markdown = ""
    if len(scan_result.vulnerabilities) > 0:
        ...
        for (
            ...
            package_name = orchestrator.extract_package_name(vuln.installed_ver)
...
def create_vulnerability_details_table(vulns: list[pkg_vuln.Vulnerability]):
    ...
    details_table_header = [
        ...
        generate_markdown_row(
            *["-" * 7 for i in range(len(details_table_header_columns))]
        ),
    ]
